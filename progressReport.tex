...
\end{figure} 
%
Figure \ref{figure:literatureReview:ribletPerformanceRegions} indicates three operating regions for the performance of a typical ribletted surface, whereby the difference in wall shear stress is plotted against the dimensionless riblet spacing, $s^+ = s * u_\tau / \nu$. The viscous regime exists for riblets with a spacing of $s^+  \lesssim 15$ whereby the drag reduction scales linearly with spacing. The riblets act to impede cross-flow leading to a reduced susceptibility to secondary motion and therefore turbulent mixing \citep{boomsma2015}. 
%
\begin{figure}[!b]
\centering
