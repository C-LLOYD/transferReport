...
LDA is a non-intrusive method of determining a fluids velocity by measuring the Doppler shift of laser light (reference the LDA book). This particular work makes use of a dual beam LDA configuration which relies on the superposition of two light waves that are differently shifted by the Doppler effect. This superposition creates the fringe pattern of Figure \ref{figure:experiments:fringePattern}, where $E_a$ and $E_b$ represent the two light waves,
\begin{equation}
E_a = E_0 \cos {\left( \omega_a t + k_a x \sin{\alpha} \right)}, \hspace{0.5cm} E_b = E_0 \cos{ \left( \omega_b t + k_b x\sin{\alpha} \right)}.
\end{equation}
