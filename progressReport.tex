...
As discussed in Section \ref{section:literatureReview}, little work has been carried out in analysing the fluid flow around shark scale surfaces; most work has focussed on the use of force balances which cannot measure velocity or pressure fields. While previous experiments have successfully measured drag reduction for shark scale surface they have not yet investigated the fluid dynamic aspects that are causing it. 

For this reason we choose to adopt Laser Doppler Anemometry (LDA) for a channel flow, with and without shark scales. A channel flow configuration allows validation against a wide range of empirical and numerical literature data and also ensures the results are applicable to a wide range of surface flows such as boat hulls.


		-	channel flow:	simple config, lots to validate against, very standard flow regime: can be applied to other problems.
			-	allows matching to CFD  - validation 

-For this reason we choose to carry out LDA over shark scale surfaces in order to both investigate how the flow fields change when compared to flat surfaces, and to provide validation for numerical models. 

\vspace{1cm}


-link to lit. review - it is clear from the literature that there is little to no data available about the flow field around shark scales, therefore we do these experiments...
...
This section discusses the methodology and results of a turbulent channel flow experiment, carried out using Laser Doppler Anemometry (LDA). The purpose of these preliminary experiments is to validate the current design before carrying out extensive work on shark scale surfaces. By measuring a simple channel flow we can compare data against numerical models, empirical models, and other laboratory experiments. 
-this will allow us to make changes for future work

\vspace{2cm}
Purpose of experiments
	-	measure a real flow around shark scales
	-	Velocity profiles, turbulence stats and friction coeffs
	-	Compare these to smooth surfaces
...
This section details the methodology and results of preliminary channel flow experiments, the purpose of which are to validate the current experimental design, and if applicable, provide information as to what must be changed for future experiments. 

In order to validate numerical models the specified boundary conditions must match those of the experiments. In particular, for periodicity to be exploited, the experiments must allow the flow to develop fully before measurements are taken.

In addition to this, the preliminary experiments will allow post-processing methods, such as filtering, to be developed.
