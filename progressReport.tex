...
LDA is a non-intrusive method of determining a fluids velocity by measuring the Doppler shift of laser light (reference the LDA book). This particular work makes use of a dual beam LDA configuration which relies on the superposition of two light waves that are differently shifted by the Doppler effect. This superposition creates the fringe pattern of Figure \ref{figure:experiments:fringePattern}. The light waves, $E_a$ and $E_b$ are defined by
\begin{equation}
E_a = E_0 \cos {\left( \omega_a t + k_a x \sin{\alpha} \right)},
\end{equation}
and
\begin{equation}
E_b = E_0 \cos{ \left( \omega_b t - k_b x\sin{\alpha} \right)},
\end{equation}
where $E_0$ represents the wave amplitude, $\omega = 2 \pi / T$ the respective angular frequencies and $k = 2 \pi / \lambda$ the angular wavenumber.
